...
**User Story**|**Test**|**Result**|**Evidence**|**Project Goals**
:---|:---|:----:|:----------:|:------|
As a **Site Owner** I can **access the project README** so that **all of the necessary information for the tool is available on project completion**<br><span style="font-size:1.5em;">[🔗](https://github.com/johnamdickson/portfolio-project-5/issues/78)</span>|README completed and uplodaed to repo.| ✅| [README](/README.md) | A modern,responsive and stylish website.
...
As a **Site Admin** I can **complete manual testing** so that **all aspects of the project will be tested.**<br><span style="font-size:1.5em;">[🔗](https://github.com/johnamdickson/portfolio-project-5/issues/80)</span>|Manual testing completed and referenced in this markdown.| ✅| [Manual Testing](#manual-testing) | A modern,responsive and stylish website.
...
|Add image - thumbnail update|When an image is added using the Choose File button the image thumbnail placeholder should be replaced with a preview of the image selected.|Clicked the Choose File button and selected an image from file|The image thumbnail updated to display the selected image.|✅|
 ...
 **Feature** | **Expected Outcome** | **Testing Performed** | **Testing Outcome** | **Result** |
|:-----|:------|:------|:-----|:------:|
|Edit product form - successful submission|When the Update Product button is clicked with all required fields in the form populated, the product detail page for the edited product should load with a success notification to the user. The product in the database should reflect the changes made in the edit which will be rendered on the product detail page.|One the *Dino Hat* product detail page clicked the Edit Product button. In the description, changed `adventurers` to `fossil hunters` before clicking the update button|The product detail page for the *Dino Hat* product loaded and a success toast appeared informing the user that the product had been successfully edited. The update was confirmed on the page in the description and also in the database by checking the products list in Django admin.|✅|
|Edit product form - required fields missing|When the Edit Product button is clicked and required fields in the form are not populated the form should not be submitted, the missing input becomes focused. A browser generated popover should appear on the missing field informing the user of the emission.|In the *Dino Hat* edit page, removed the item price and clicked on the Update Now button.|The Edit product form was not submitted, the missing input focused and a browser generated popover appeared informing the user of the omission.|✅|
|Edit product form - data persistence|When the Edit Product page loads the existing data for the product should be prepopulated in the form.|Loaded the edit product page for *Dino Hat*|All of the existing text and selections persisted in the rendered form.|✅|
|Edit product form - secondary colour checkbox conditional formatting|When the Edit Product page loads and the existing product has no colours attribute, the secondary solour checkbox should be disabled. |Accessed the Edit Product page for the *Newborn Hat Tutorial.|On load of the Edit Product page, the secondary colour checkbox was disabled.|✅|
|Edit product form - no existing image.|If a product does not have an image associated with it, a placeholder should be used in its abscence on the edit product form.|Added a test product with all required fields and no image before accessing its edit product page.|The No Image Selected placeholder appeared as the image thumbail. |✅|
|Edit product form - existing image persistence.|If a product has an image associated with it, it should render in the form image thumbnail.|Accessed the Edit Product page for *Dino Hat*.|The existing image appeared as the image thumbail. |✅|
|Edit product form - learn product checkbox conditional formatting|When the learn product checkbox is checked, on loading of the edit product page the checkbox should be checked, the input for adding a file (along with option to change or clear) should be visible and the sizes and primary colour checkboxes become disabled. When the checkbox is deselected, the input for adding a file should disappear and  the sizes and primary colour checkboxes should be re-enabled. If any of the sizes or colours checkboxes are selected, the learn product checkbox should be disabled. If all of the sizes and colours checkboxes are deselected the checkbox should be enabled. |Loaded the *Dino Hat* edit product page, selected the learn product checkbox before deselecting again. The next step was to select a size and primary colour checkbox before deselecting again.|On loading of the edit product page, the learn product checkbox was selected and the learn file upload selection along with option to change and claer was visible. All other product attribute checkboxes were disabled. On deselecting the learn product checkbox the learn file upload disappeared and both sizes and colours checkboxes re-enabled. On selection of sizes and colours checkboxes, the learn product checkbox was disabled, renabling when all sizes and colours checkboxes were deselected. |✅|
|Change image|When an image is changed using the Choose File button the image thumbnail placeholder should be replaced with the new image preview. On submission of the edit product form, the new image should be present in the product detail page.|On the edit product page for the test product added earlier, clicked the Choose File button and selected a new image from file|The image thumbnail updated to display the selected image. On form submission, the product detail page for the test product loaded and the new image displayed as expected.|✅|
|Delete Product button - confirm action cancelled|On clicking the Delete Product button the user should be presented with a confirm dialogue. Clicking cancel should take no action other than dismissing the dialogue.| Clicked a test product Delete Product button and then clicked cancel on the resultant dialogue.| Dialogue dismissed with not further outcomes.|✅|
|Delete Product button - confirm action|On clicking the Delete Product button the user should be presented with a confirm dialogue. Clicking OK should load the products page where the deleted item should no longer exist. A product deleted toast should appear informing the user that they have deleted the product.| Clicked the test product Delete Product button and then clicked OK on the resultant dialogue.| Products page loaded and deleted product was no longer present. An success toast appeared with the correct information.|✅|
|Cancel link|When the cancel link is clicked the user should be returned to the products page.|Clicked the cancel link adjacent to Delete Product button|The page returned to the products page.|✅
...
## Bugs
The were 21 recorded bugs encountered during the project. Of the 21, 19 were resolved and 2 remain unresolved. All of the bugs are summarised in the table below.

 **Bug** | **Description** | **Actions to Resolve** | **Outcome** |
|:-----|:------|:------|:-----|
|[Hover pseudo class issue post click](https://github.com/johnamdickson/portfolio-project-5/issues/18)|All header links associated with an offcanvas element display a common error whereby the hover psuedo class does not work after the link is clicked.|Added !important to end of hover pseudo class which superseded the whichever class was creating the problem. I tried to find a less brute force solution in stack overflow and spent a bit of time investigating in Chrome devtools but decided in the interests of project timeline, the solution is satisfactory.|Resolved|
|[ Querying categories in product detail](https://github.com/johnamdickson/portfolio-project-5/issues/31)|Whilst attempting to add functionality to search categories along with product name and descriptions using q, the following error was returned: *Related Field got invalid lookup: icontains*|The icontains lookup notation requires double underscores after the foreign key name as opposed to dot notation. Solution found in <a href="https://stackoverflow.com/questions/11754877/troubleshooting-related-field-has-invalid-lookup-icontains">stack overflow</a>|Resolved|
|[Products missing from nav drop downs](https://github.com/johnamdickson/portfolio-project-5/issues/32)|Product drop down items empty on other categories when one particular category is selected. For example, when the learning category is filtered for the hat products do not appear in the hats drop down and vice versa. This happens because the main products queryset is filtered by category and removes the other categories.|Added an unfiltered_products property to the products and product-detail views which resolved the issue and the correct products are appearing in all envisaged category filtering.|Resolved|
|[Console error on home page](https://github.com/johnamdickson/portfolio-project-5/issues/33)|On the home page the following console error was generated: *Cannot read properties of undefined (reading 'getElementsByClassName')*  The error occurs because the drop down menu is purposely hidden on the home page. |Added logic to check pathname and only complete code when not on the home page.|Resolved|
|[Navbar menu bugs on cart page.](https://github.com/johnamdickson/portfolio-project-5/issues/37)|Observed 2 bugs on cart page navbar dropdown menus. The first bug encountered was that non-category items were missing from the menus. The second bug was the lack of any custom styling on the navbar and dropdown menus which is present in the other pages.|The first bug was resolved by including unfiltered_products in the cart view context. The second bug was took a little more time to diagnose and rectify. I was able to resolve it by removing the `{% block postloadjs %}` block. Having this block in place seems to require the `{{ block.super }}` to be called to run the script, regardless if main block is empty.|Resolved|
|[Cart quantity buttons not functioning](https://github.com/johnamdickson/portfolio-project-5/issues/38)|Javascript function to increment and decrement item quantities not working.|Further investigation revealed that there was an additional element returned in the array for form inputs. This element was the search bar as the class name used to get elements was input-group. Changed to a more specific class name call and bug resolved.|Resolved|
|[Elements with non-unique IDs console warning](https://github.com/johnamdickson/portfolio-project-5/issues/45)|Received warnings for non-unique IDs after implementing login via offcanvas functionality.|Created a bespoke offcanvas form for small screen and added `_1` to each of the offending tags which removed the warnings whilst retaining the log in functionality.|Resolved|
|[Empty cart stopped returning correct message](https://github.com/johnamdickson/portfolio-project-5/issues/48)|The empty cart no longer returned a message for user that it was empty rather returning the order summary. Checked cart items which was returning a reverse iterator. This happened as a result of earlier attempts to order the cart items so that newer ones are shown first.|Changed logic to check that length of cart items is >0 as the reverse iterator returns an empty array.|Resolved|
|[Order post error on checkout submission](https://github.com/johnamdickson/portfolio-project-5/issues/54)|On submission of form and payment, the order and order line item were not updating in the DB.|Noted that there was no Post call on submission of button due to the JS code for Stripe handle payment. Made post call from JS to checkout url with the required data fed into a FormData object.|Resolved|
|[Order shipping data bug](https://github.com/johnamdickson/portfolio-project-5/issues/55)|When attempting to update shipping data in payment intent webhook, received an unknown error message.|Issue traced back to Stripe webhook not recognising country long form terminology. Resolved by adding country field to Object model and returning correct input from DOM to Stripe via JS code.|Resolved|
|[Order shipping data bug](https://github.com/johnamdickson/portfolio-project-5/issues/59)|On profile form submit, started to receive errors that the form was not valid.|Issue arose after the user was added as a form field. Attempted to make it read only by setting disabled but this caused form to be invalid as it is a required field. Resolved by copying HTML for one of the other `label` and `input` and passing in user.username before setting element as `readonly`.|Resolved|
|[Grand total error](https://github.com/johnamdickson/portfolio-project-5/issues/72)|Order grand total was not updating in production environment.|Found solution on Slack whereby signals do not work in Heroku. Solution was to add weak=False as argument in receiver decorators.|Resolved|
|[Email sending bug](https://github.com/johnamdickson/portfolio-project-5/issues/75)|Received 500 server error when trying to sign up and test the email verification. Set debug to true which revealed the error as:*[SMTP.starttls() got an unexpected keyword argument 'keyfile'](https://stackoverflow.com/questions/77482831/smtp-starttls-got-an-unexpected-keyword-argument-keyfile)*|Tried searching for a straight forward solution that would enable the verification email to send error free however all guidance online pointed towards a requirement to upgrade Django to version 4.2.7, as explicitly mentioned in this [Stack Overflow post](https://stackoverflow.com/questions/77482831/smtp-starttls-got-an-unexpected-keyword-argument-keyfile). Decided to proceed with upgrade which resolved the issue.|Resolved|
|[Learn e-mail failing to send](https://github.com/johnamdickson/portfolio-project-5/issues/84)|Learn to crochet email with attachment working OK in development environment but failed to work in production environment.|Added print statements and try/except block into function to establish root cause of issue. The following error was returned: `Trying to create path returned this error: This backend doesn't support absolute paths.`The issue related to creation of path for base64 encoding of Path object. Found solution on [Stack Overflow](https://stackoverflow.com/questions/49324010/how-to-encode-a-file-in-filefield-using-s3-to-base64-in-django) which resolved the issue.|Resolved|
|[Form submission CSRF bug](https://github.com/johnamdickson/portfolio-project-5/issues/85)|Issue occurred during final mentor session whereupon it became obvious that there was an issue with form submission. Each time my mentor tried to submit a form he received a 403 error with a 403 error of `CSFR cookie not set`.|Issue resolved after a lengthy investigation process by removing the CSRF_COOKIE_DOMAIN property from settings.|Resolved|
|[General bugs with cart](https://github.com/johnamdickson/portfolio-project-5/issues/86)|Observed abnormal cart information across various pages and inability to correctly adjust or delete items from cart on cart page.|Issue related to adopting site wide caching without considering effect on session being updated in the templates. Opted instead for template specific caching which resolved the issues.|Resolved|
|[500 Server Error - user id](https://github.com/johnamdickson/portfolio-project-5/issues/87)|Encountered the following 500 server error `Field 'id' expected a number but got <SimpleLazyObject: <django.contrib.auth.models.AnonymousUser object at 0x7fccc73e1eb0>>` | had recently refactored the checkout app views file, specifically the instantiation of user profiles and checking if data exists.  However, the try except block for checking if there was user profile existed outside check that user is authenticated. The error was caused when looking for an user id where none existed with anon user, raising a TypeError. I resolved this by including a TypeError clause.|Resolved|
|[Profile form update error](https://github.com/johnamdickson/portfolio-project-5/issues/91)|When testing profile form, data was deleted on update.|Issue related to amendments made to address duplicate IDs across main view and both off canvas profile update forms. Resolved by adding conditional logic to check which form is posting and then instantiating form with the correct prefix.|Resolved|
|[Profile form update error](https://github.com/johnamdickson/portfolio-project-5/issues/92)|Deploy failed with error *Compiled slug size: 566.7M is too large (max is 500M).*|Added .slugignore file and included documentation directory which allowed the deployment to process.|Resolved|
|[Failure to deploy to Heroku](https://github.com/johnamdickson/portfolio-project-5/issues/92)|Deploy failed with error *Compiled slug size: 566.7M is too large (max is 500M).*|Added .slugignore file and included documentation directory which allowed the deployment to process.|Resolved|
|[mpty required field popover not appearing as expected](https://github.com/johnamdickson/portfolio-project-5/issues/93)|When a required field is not completed and pay now button pressed the form does not submit and the window scrolls to and highlights the missing input. The standard browser popover does not appear however.|Given the form is not submitted and the window scrolls to and highlights the input required the bug will remain unresolved with a view to addressing in future versions of the app.|Unresolved|
|[Learn Product Delivery Charge](https://github.com/johnamdickson/portfolio-project-5/issues/94)|When the user purchases only a Learn Product delivered via email they are charged for delivery.|No action taken due to timeline for project completion. To be actioned in later version of the app|Unresolved|
